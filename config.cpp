...
int cfg::read_army_units(std::map<char, std::vector<int>>& army_units, std::string unit_string)
{
...
int cfg::read_config(std::map<std::string, int>& basic_params,
...
{
    std::ifstream config_file{ "simulation.config" };
    ...
    if (config_file.fail()) {
        std::cout << "Error, config file unreadable" << std::endl;
        return -1;
    }
        ...
        if (line[0] != '#' && line != "") {
            ...
            if (*end_of_numeric || value == "") {
                ...
                //Unpack unit strings, if present
                    ...
                    load_status = cfg::read_army_units(army_1_units, value);
                    if (load_status != 0) return -1;
                }
                else if (key == "army_2_units") {
                    load_status = cfg::read_army_units(army_2_units, value);
                    if (load_status != 0) return -1;
                }
                else if (key == "tileset") {
                    load_status = cfg::read_army_units(tileset_tiles, value);
                    if (load_status != 0) return -1;
...
int cfg::populate_army(std::shared_ptr<army>& empty_army, std::string army_name, std::map<char, std::vector<int>>& army_units)
{
    std::vector<int> unit_coords{};
    ...
    for (auto pair : army_units) {
        ...
        switch (pair.first) {
        case('S'):
            for (coord_iter = unit_coords.begin(); coord_iter < unit_coords.end() - 1; coord_iter += 2) {
                empty_unit.reset(new red_sw(*coord_iter, *(coord_iter + 1)));
                empty_army->add_unit(empty_unit);
            }
            break;
        case('A'):
            for (coord_iter = unit_coords.begin(); coord_iter < unit_coords.end(); coord_iter += 2) {
...
int cfg::load_simulation(std::unique_ptr<battlefield>& field, int& tick_length, int& max_iterations)
{
    ...
    read_status = cfg::read_config(basic_params, army_names, army_1_units, army_2_units, tileset_tiles);
    if (read_status != 0) {
    ...
    read_status = cfg::populate_army(army_1, army_names["army_1_name"], army_1_units);
    if (read_status != 0) {
    ...
    read_status = cfg::populate_army(army_2, army_names["army_2_name"], army_2_units);
    if (read_status != 0) {
    ...
    read_status = cfg::populate_army(tileset, "tileset", tileset_tiles);
    if (read_status != 0) {
